...
from utils import (
    validate_dataframe, safe_divide, clean_numeric_data,
...
def compute_rsi(prices: pd.Series, period: int = 14) -> pd.Series:
    ...
    Returns:
        RSI values (0-100 range, NaN filled with 50 for neutral)
        
    Raises:
        ValueError: If prices series is empty or period is invalid
    """
    if len(prices) == 0:
...
def compute_macd(prices: pd.Series, fast: int = 12, slow: int = 26, signal: int = 9) -> pd.Series:
    ...
    Returns:
        Normalized MACD signal (positive = bullish, negative = bearish)
    """
    ...
    return safe_divide(macd_line - signal_line, prices, fill_value=0.0)
...
def compute_bollinger_position(prices: pd.Series, period: int = 20, std_dev: int = 2) -> pd.Series:
    ...
    Returns:
        Position from -1 (at lower band) to 1 (at upper band), 0 = neutral
    """
    ...
    position = safe_divide(2 * (prices - lower_band), bb_width, fill_value=0.0) - 1
...
def compute_atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14) -> pd.Series:
    ...
    Returns:
        Normalized ATR (relative to close price)
    """
    ...
    return safe_divide(atr, close, fill_value=0.0)  # Normalize by price
...
def compute_obv_change(close: pd.Series, volume: pd.Series, period: int = 10) -> pd.Series:
    ...
    Args:
        ...
        period: Change calculation period (default: 10)
    ...
    return clean_numeric_data(obv_change, replace_inf=True, fill_na=0.0)
...
def build_enhanced_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if len(df) < 50:
            raise ValueError(
                f"Input OHLCV data has {len(df)} rows, minimum 50 required for reliable feature computation"
            )
        ...
        out["dd_20"] = safe_divide(close, roll_max, fill_value=1.0) - 1.0
        
        # Range
        out["range_pct"] = safe_divide(high - low, close, fill_value=0.0)
        
        # RSI
        out["rsi_14"] = safe_divide(compute_rsi(close, 14) - 50, 50, fill_value=0.0)
        ...
        momentum_denom = close.shift(10).rolling(3).mean()
        out["momentum_10"] = safe_divide(close, momentum_denom, fill_value=1.0) - 1
        ...
        out["ema_ratio"] = safe_divide(ema_12, ema_26, fill_value=1.0) - 1
        
        # Volume trend
        vol_sma = volume.rolling(20).mean()
        out["volume_sma_ratio"] = safe_divide(volume, vol_sma, fill_value=1.0) - 1
        ...
        raise FeatureEngineeringError(f"Failed to build enhanced features: {str(e)}") from e
...
def create_target(df: pd.DataFrame, horizon: int = 1) -> pd.Series:
    ...
    Raises:
        ValueError: If close column is missing or horizon is invalid
    """
    if horizon < 1:
    ...
    close = df["close"].astype(float)
    future_return = safe_divide(close.shift(-horizon), close, fill_value=1.0) - 1
...
def prepare_train_data(
    ...
    try:
        ...
        if len(combined) == 0:
            raise FeatureEngineeringError("No valid samples after feature engineering and target creation")
        ...
        raise FeatureEngineeringError(f"Training data preparation failed: {str(e)}") from e
